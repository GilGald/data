...
#if no env variables 
if [ -z "$1" ] || [ -z "$2" ] || [ -z "$3" ]; then
	docker run -d --name guacamole-client --link guacamole-server:guacd -p 8080:8080 quali/guacamole-client:1.0.0
else 
	#bucket name ,region,key name
	docker run -d --name guacamole-client --link guacamole-server:guacd -p 8080:8080  -e $1 -e $2 -e $3 quali/guacamole-client:1.0.0
fi
